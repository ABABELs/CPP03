...
/*   Updated: 2023/12/13 18:36:11 by babels           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    std::cout << "ClapTrap " << this->_name << " take " << amount << " points of damage to " << this->getEnnemyName() << "! You have Now: " << (_hit_points -= amount) << " hit points !" << std::endl;
}
...
std::string ClapTrap::getEnnemyName(void) const
{
    return (_ennemy_name);
}

void    ClapTrap::choose_Name_Ennemy(std::string name)
{
    _ennemy_name = name;
}